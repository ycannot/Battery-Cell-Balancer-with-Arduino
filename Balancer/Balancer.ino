...
float inputNumber=8, temporary, average;
byte analog_inputs[]={A0,A1,A2,A3};
int digital_outputs[]={2,3,4,5,6,7,8,9};
...
void loop() {
  ...
  for (int i=0; i<inputNumber; i++){
    temp=0;
    for (int j=0; j<8;j++){
      temp+=((float)(analogRead(analog_inputs[k+i]))*voltage_divs[i]*5/1023)-temporary;
      delay(1);
    }
    voltages[i] = temp/8;
    temporary += voltages[k+i];
    Serial.print(k+i+1);
    Serial.print(":");
    Serial.println(voltages[k+i]);
    Serial.println();
  }
  Serial.println("---------");
  ...
  for (int i=0; i<inputNumber; i++){
    if (voltages[k+i] - average > cell_ave_interval){
      digitalWrite(digital_outputs[k+i],HIGH); 
    }
    
    else{
      digitalWrite(digital_outputs[k+i],LOW);
    }
